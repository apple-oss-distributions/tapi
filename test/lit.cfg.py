...
config.tapi_json_diff = infer_tapi_tool("api-json-diff", "TAPI_JSON_DIFF", config.environment['PATH']).replace('\\', '/')
config.readtapi = infer_tapi_tool("llvm-readtapi", "READTAPI", config.environment['PATH']).replace('\\', '/')
# Remove for OPEN_SOURCE scrub.
config.tapi_mrm = infer_tapi_tool("tapi-mrm", "TAPI_MRM", config.environment['PATH']).replace('\\', '/')
config.tapi_analyze = infer_tapi_tool("tapi-analyze", "TAPI_ANALYZE", config.environment['PATH']).replace('\\', '/')
# end of removal
...
config.substitutions.append( ('%api-json-diff', config.tapi_json_diff) )
config.substitutions.append( ('%tapi-sdkdb', config.tapi_sdkdb) )
config.substitutions.append( ('%readtapi', config.readtapi) )
# Remove for OPEN_SOURCE scrub.
config.substitutions.append( ('%tapi-mrm', config.tapi_mrm) )
config.substitutions.append( ('%tapi-analyze', config.tapi_analyze) )
# end of removal
...
config.substitutions.append(
    ('%hmaptool', "'%s' %s" % (config.python_executable,
                             os.path.join(config.clang_src_root, "utils", 'hmaptool', 'hmaptool'))))

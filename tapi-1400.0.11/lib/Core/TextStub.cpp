...
template <> struct ScalarTraits<PlatformSet> {
  static void output(const PlatformSet &value, void *io, raw_ostream &out) {
    ...
    // obsoleted by TBD v4.
    if (value.count(PlatformKind::macOS) &&
        value.count(PlatformKind::macCatalyst)) {
  ...
  static StringRef input(StringRef scalar, void *io, PlatformSet &value) {
    ...
    if (scalar == "zippered") {
      if (ctx && ctx->fileType >= TBDv3) {
        value.insert(PlatformKind::macOS);
        value.insert(PlatformKind::macCatalyst);
        return {};
      }
    ...
    if (platform == PlatformKind::macCatalyst ||
        platform == PlatformKind::driverKit)
      if (ctx && ctx->fileType < TBDv3)
        return "unknown enumerated scalar";

...
enum Bar {
  ...
  // CHECK:      - name: Foo::A
  // CHECK-NEXT:   USR: c:@N@Foo@E@Bar@A
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  A,

  // CHECK:      - name: Foo::B
  // CHECK-NEXT:   USR: c:@N@Foo@E@Bar@B
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  B,

  // CHECK:      - name: Foo::C
  // CHECK-NEXT:   USR: c:@N@Foo@E@Bar@C
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  C,
};
}
...
enum Bar {
  // CHECK-LABLE: constants
  // CHECK:      - name: Baz::A
  // CHECK-NEXT:   USR: c:@N@Baz@E@Bar@A
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  A,

  // CHECK:      - name: Baz::B
  // CHECK-NEXT:   USR: c:@N@Baz@E@Bar@B
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  B,

  // CHECK:      - name: Baz::C
  // CHECK-NEXT:   USR: c:@N@Baz@E@Bar@C
  // CHECK-NEXT:   loc: {{.*}}/scoped_enums.cpp:[[@LINE+1]]:3
  C,
};
}
